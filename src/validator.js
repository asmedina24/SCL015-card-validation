 ...
 const validator = {    
   isValid: (creditCardNumber) => {
    ...
    for (let i = 0; i < creditCardNumber.length; ++i) {  // recorre el arreglo
      let numero= parseInt(creditCardNumber.charAt(i));   // parsea los numero de las tarjeta  de texto a numero y  evuelve el carácter situado en la posición 'index' pasada por parámetro. La primera posición es la 0.
        ...
        if(multiplica > 9) {    // si el numero es mayor a 9
         ...
         multiplica = parseInt(multiplicaCadena.charAt(0))+ parseInt(multiplicaCadena.charAt(1)); //parsea el numero y extrae un numero de posicion
    ...
    let escondido = "*";                          //se crea una variable que almacena el string
    let tarjetaEnmascarada = creditCardNumber.slice(-4);  // se crea una variable que tome el creditcard menos los ultimos 4
